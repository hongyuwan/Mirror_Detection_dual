...
bce = nn.BCEWithLogitsLoss().cpu()


def main():
    print(args)

    net = BASE6(backbone_path).cpu().train()
    if args['add_graph']:
        writer.add_graph(net, input_to_model=torch.rand(
            args['train_batch_size'], 3, args['scale'], args['scale']).cpu())
...
def train(net, optimizer):
    ...
    for epoch in range(args['last_epoch'] + 1, args['last_epoch'] + 1 + args['epoch_num']):
        ...
        for data in train_iterator:
            ...
            # [Lb: % .5f],
            log = '[Epoch: %2d], [Iter: %5d], [%.7f], [Sum: %.5f], [Lc: %.5f], [Lo: %.5f]' % \
                  (epoch, curr_iter, base_lr, loss_record.avg, loss_c_record.avg, loss_o_record.avg)
            # loss_b_record.avg,
